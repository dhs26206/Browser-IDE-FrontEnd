                ...
                {list.map((item,index)=>{
                    ...
                    return (<div id={item.contest_id} key={index} className="px-6 flex w-full h-36 flex-wrap  border-2 rounded-2xl border-gray-700 hover:border-4">
                        ...
                        <div className="w-1/2 flex justify-end h-[90%] items-center">
                            <div>
                                <button onClick={!(isContestEnded || !isContestActive)?()=>handleNavigateContest(item.contest_id):""} className={`rounded-md px-7 py-4 text-white ${isContestEnded || !isContestActive? 'bg-gray-400 cursor-not-allowed': 'bg-blue-500 hover:bg-blue-700'} `}>
